...
def mito_assign_skeleton(image_path, excel_path, trunk_node_path, save_excel_path, Pixel, left_position):
    ...
    v_per = 0.07 * 1000 * 1000 * 1000  # unit volume (nm^3)
    ...
    with pd.ExcelWriter(save_excel_path) as writer:
        for name in tqdm(xls.sheet_names):
            ...
            coord_trunk = np.array(trunk_node.iloc[:, :3])  # coord of ANF skeleton
            L = len(data)
            new_data = []
            for i in range(L):
                #  获取线粒体的中点
                id_mito = int(data.iloc[i, 3].split('-')[1])  # id_mito

                v_mito_time = int(data.iloc[i, 5] / v_per) + 1  # the times of mitochondira volume (unit volume)
                ...
                if v_mito_time == 1:
                    ...
                    central_mito = central_mito[::-1]  # One-dimensional array inversion
                    central_mito = central_mito.reshape(1, -1)

                    # print("compute distance")
                    dist = calculate_distance_3d(central_mito, coord_trunk, Pixel)
                    # dist = np.min(dist, axis=1)
                    index_node = np.argmin(dist)  # index of minimum distance
                    ...
                    maxindex = np.argmax(d_v)  # Longest dimension (X or Y or Z)
                    list_mito = []
...
if __name__ == "__main__":
    ...
    Pixel = [12, 12, 50]
    # w_0, h_0, z_0 = 2100, 5900, 0  # The difference between webknossos and downloading images for the starting point.  HuaLab-base-CBA-wt
    # w_0, h_0, z_0 = 4100, 6500, 650  # The difference between webknossos and downloading images for the starting point.  HuaLab-CBA-wt-base-002
